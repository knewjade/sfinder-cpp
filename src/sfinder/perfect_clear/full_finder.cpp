...
namespace sfinder::perfect_clear {
    // For PC-able search
    void Recorder<PCableCandidate, PCableRecord>::clear() {
        best_ = PCableRecord{
                std::vector<Operation>{},
        };
    }

    void Recorder<PCableCandidate, PCableRecord>::update(const Solution &solution, const PCableCandidate &current) {
        best_ = PCableRecord{
                solution,
        };
    }

    bool Recorder<PCableCandidate, PCableRecord>::isWorseThanBest(
            bool leastLineClears, const PCableCandidate &current
    ) const {
        return !best_.solution.empty();
    }

    bool Recorder<PCableCandidate, PCableRecord>::shouldUpdate(
            bool leastLineClears, const PCableCandidate &newRecord
    ) const {
        return true;
    }
